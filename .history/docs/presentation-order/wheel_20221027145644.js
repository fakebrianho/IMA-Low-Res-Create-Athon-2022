class Wheel {
	constructor(parentID, data, count = 5, startIndex = 0) {
		...
		this.currentIndex =
			this.startIndex - this.data.length * this.numOfCycles
		...
		for (let i = 0; i < this.count; i++) {
			this.elements[i] = createP(
				this.data[
					(this.startIndex + i - floor(this.count / 2)) %
						this.data.length
				]
			)
			...
			this.elements[i].style(
				'color',
				rgbToHex(int(random(120)), int(random(120)), int(random(120)))
			)
	...
	move() {
		...
		this.speed =
			(60 *
				abs(
					this.difference % (this.data.length * this.numOfCycles * 2)
				)) /
			(this.data.length * this.numOfCycles * 2)
		...
		if (this.currTop < -this.distanceBetweenElements / 108) {
			clickSound.play()
			this.currentIndex++
			this.startIndex =
				this.currentIndex + this.data.length * this.numOfCycles
			...
			this.elements[this.elements.length - 1].attribute(
				'aria-hidden',
				true
			)
			this.elements[this.elements.length - 1].style(
				'color',
				rgbToHex(int(random(120)), int(random(120)), int(random(120)))
			)
		}
